  ...
  : nh1_(nh1), nh2_(nh2), nh3_(nh3), nh4_(nh4), nh5_(nh5)
{
  ...
  safeGetParam(nh1_, "deployment/replanning_trigger_time_student", par_.replanning_trigger_time_student);
  safeGetParam(nh1_, "deployment/replanning_trigger_time_expert", par_.replanning_trigger_time_expert);
  safeGetParam(nh1_, "deployment/pause_time_when_replanning", par_.pause_time_when_replanning);
  safeGetParam(nh1_, "deployment/replanning_lookahead_time", par_.replanning_lookahead_time);
  safeGetParam(nh1_, "deployment/use_mesh_network", par_.use_mesh_network);
  safeGetParam(nh1_, "deployment/use_delaycheck", par_.use_delaycheck);
  safeGetParam(nh1_, "deployment/use_delaycheck_wo_check", par_.use_delaycheck_wo_check);
  safeGetParam(nh1_, "deployment/delaycheck_time", par_.delaycheck_time);
  safeGetParam(nh1_, "deployment/use_panther_star", par_.use_panther_star);
  safeGetParam(nh1_, "deployment/impose_FOV_in_trajCB", par_.impose_FOV_in_trajCB);
  safeGetParam(nh1_, "deployment/dist_from_gterm_to_dummy", par_.dist_from_gterm_to_dummy);
  safeGetParam(nh1_, "deployment/obstacle_share_cb_duration", par_.obstacle_share_cb_duration);
  safeGetParam(nh1_, "deployment/use_obstacle_share", par_.use_obstacle_share);
  safeGetParam(nh1_, "deployment/use_obstacle_shareCB", par_.use_obstacle_shareCB);
  safeGetParam(nh1_, "deployment/add_noise_to_obst", par_.add_noise_to_obst);
  safeGetParam(nh1_, "deployment/agents_ids", par_.agents_ids);
  safeGetParam(nh1_, "deployment/use_ff", par_.use_ff);
  safeGetParam(nh1_, "deployment/goal_radius", par_.goal_radius);
  safeGetParam(nh1_, "deployment/goal_seen_radius", par_.goal_seen_radius);
  safeGetParam(nh1_, "deployment/Ra", par_.Ra);
  std::vector<double> drone_bbox_tmp;
  safeGetParam(nh1_, "deployment/drone_bbox", drone_bbox_tmp);
  par_.drone_bbox << drone_bbox_tmp[0], drone_bbox_tmp[1], drone_bbox_tmp[2];
  std::vector<double> obstacle_bbox_tmp;
  safeGetParam(nh1_, "deployment/obstacle_bbox", obstacle_bbox_tmp);
  par_.obstacle_bbox << obstacle_bbox_tmp[0], obstacle_bbox_tmp[1], obstacle_bbox_tmp[2];
  safeGetParam(nh1_, "deployment/dc", par_.dc);
  safeGetParam(nh1_, "deployment/angle_deg_focus_front", par_.angle_deg_focus_front);
  safeGetParam(nh1_, "deployment/x_min", par_.x_min);
  safeGetParam(nh1_, "deployment/x_max", par_.x_max);
  safeGetParam(nh1_, "deployment/y_min", par_.y_min);
  safeGetParam(nh1_, "deployment/y_max", par_.y_max);
  safeGetParam(nh1_, "deployment/z_min", par_.z_min);
  safeGetParam(nh1_, "deployment/z_max", par_.z_max);
  safeGetParam(nh1_, "deployment/ydot_max", par_.ydot_max);
  std::vector<double> v_max_tmp;
  std::vector<double> a_max_tmp;
  std::vector<double> j_max_tmp;
  safeGetParam(nh1_, "deployment/v_max", v_max_tmp);
  safeGetParam(nh1_, "deployment/a_max", a_max_tmp);
  safeGetParam(nh1_, "deployment/j_max", j_max_tmp);
  ...
  safeGetParam(nh1_, "deployment/factor_alpha", par_.factor_alpha);
  safeGetParam(nh1_, "deployment/lower_bound_alpha", par_.lower_bound_alpha);
  safeGetParam(nh1_, "deployment/max_seconds_keeping_traj", par_.max_seconds_keeping_traj);
  safeGetParam(nh1_, "deployment/gamma", par_.gamma);
  safeGetParam(nh1_, "deployment/use_expert", par_.use_expert);
  safeGetParam(nh1_, "deployment/use_student", par_.use_student);
  ...
  safeGetParam(nh1_, "training/num_of_trajs_per_replan", par_.num_of_trajs_per_replan);
  safeGetParam(nh1_, "training/max_num_of_initial_guesses", par_.max_num_of_initial_guesses);
  safeGetParam(nh1_, "training/drone_extra_radius_for_NN", par_.drone_extra_radius_for_NN);
  safeGetParam(nh1_, "training/use_closed_form_yaw_student", par_.use_closed_form_yaw_student);
  safeGetParam(nh1_, "training/training_env_x_max", par_.training_env_x_max);
  safeGetParam(nh1_, "training/training_env_x_min", par_.training_env_x_min);
  safeGetParam(nh1_, "training/training_env_y_max", par_.training_env_y_max);
  safeGetParam(nh1_, "training/training_env_y_min", par_.training_env_y_min);
  safeGetParam(nh1_, "training/training_env_z_max", par_.training_env_z_max);
  safeGetParam(nh1_, "training/training_env_z_min", par_.training_env_z_min);
  safeGetParam(nh1_, "training/max_dist2goal", par_.max_dist2goal);
  safeGetParam(nh1_, "training/max_dist2obs", par_.max_dist2obs);
  safeGetParam(nh1_, "training/max_side_bbox_obs", par_.max_side_bbox_obs);
  safeGetParam(nh1_, "training/max_dist2BSPoscPoint", par_.max_dist2BSPoscPoint);
  safeGetParam(nh1_, "training/training_dt", par_.training_dt);
  safeGetParam(nh1_, "training/yaw_loss_weight", par_.yaw_loss_weight);
  safeGetParam(nh1_, "training/margin_v_factor", par_.margin_v_factor);
  safeGetParam(nh1_, "training/margin_a_factor", par_.margin_a_factor);
  safeGetParam(nh1_, "training/margin_ydot_factor", par_.margin_ydot_factor);
  safeGetParam(nh1_, "training/margin_yaw_factor", par_.margin_yaw_factor);
  std::vector<double> training_obst_size_tmp;
  safeGetParam(nh1_, "training/training_obst_size", training_obst_size_tmp);
  ...
  safeGetParam(nh1_, "training/training_other_agent_size", training_other_agent_size_tmp);
  ...
  safeGetParam(nh1_, "training/use_dynamic_obst_in_training", par_.use_dynamic_obst_in_training);
  safeGetParam(nh1_, "training/use_other_agents_in_training", par_.use_other_agents_in_training);
  safeGetParam(nh1_, "training/use_expert_for_other_agents_in_training", par_.use_expert_for_other_agents_in_training);
  safeGetParam(nh1_, "training/random_num_of_obstacles_in_training", par_.random_num_of_obstacles_in_training);
  safeGetParam(nh1_, "training/num_of_obstacles_in_training", par_.num_of_obstacles_in_training);
  safeGetParam(nh1_, "training/num_of_other_agents_in_training", par_.num_of_other_agents_in_training);
  safeGetParam(nh1_, "training/use_clipping", par_.use_clipping);
  safeGetParam(nh1_, "training/use_noised_obst_size", par_.use_noised_obst_size);
  safeGetParam(nh1_, "training/goal_seen_radius_training", par_.goal_seen_radius_training);

  //
  // LSTM
  //

  safeGetParam(nh1_, "lstm/use_lstm", par_.use_lstm);
  safeGetParam(nh1_, "lstm/use_bn", par_.use_bn);
  safeGetParam(nh1_, "lstm/lstm_dropout", par_.lstm_dropout);
  safeGetParam(nh1_, "lstm/lstm_output_dim", par_.lstm_output_dim);
  safeGetParam(nh1_, "lstm/lstm_num_layers", par_.lstm_num_layers);
  safeGetParam(nh1_, "lstm/lstm_bidirectional", par_.lstm_bidirectional);

  //
  // LSTM for other agents
  //

  safeGetParam(nh1_, "lstm_oa/use_lstm_oa", par_.use_lstm_oa);
  safeGetParam(nh1_, "lstm_oa/use_bn_oa", par_.use_bn_oa);
  safeGetParam(nh1_, "lstm_oa/lstm_dropout_oa", par_.lstm_dropout_oa);
  safeGetParam(nh1_, "lstm_oa/lstm_output_dim_oa", par_.lstm_output_dim_oa);
  safeGetParam(nh1_, "lstm_oa/lstm_num_layers_oa", par_.lstm_num_layers_oa);
  safeGetParam(nh1_, "lstm_oa/lstm_bidirectional_oa", par_.lstm_bidirectional_oa);

  //
  // FOV 
  //

  safeGetParam(nh1_, "fov/fov_x_deg", par_.fov_x_deg);
  safeGetParam(nh1_, "fov/fov_y_deg", par_.fov_y_deg);
  safeGetParam(nh1_, "fov/fov_depth", par_.fov_depth);

  //
  // octopus search
  //

  safeGetParam(nh1_, "octopus/a_star_samp_x", par_.a_star_samp_x);
  safeGetParam(nh1_, "octopus/a_star_samp_y", par_.a_star_samp_y);
  safeGetParam(nh1_, "octopus/a_star_samp_z", par_.a_star_samp_z);
  safeGetParam(nh1_, "octopus/a_star_fraction_voxel_size", par_.a_star_fraction_voxel_size);
  safeGetParam(nh1_, "octopus/a_star_bias", par_.a_star_bias);
  safeGetParam(nh1_, "octopus/disc_pts_per_interval_oct_search", par_.disc_pts_per_interval_oct_search);
  safeGetParam(nh1_, "octopus/max_runtime_octopus_search", par_.max_runtime_octopus_search);
  ...
  safeGetParam(nh1_, "visualization/use_obstacle_edge_cb", par_.use_obstacle_edge_cb);
  safeGetParam(nh1_, "visualization/obstacle_edge_cb_duration", par_.obstacle_edge_cb_duration);
  safeGetParam(nh1_, "visualization/visual", par_.visual);
  safeGetParam(nh1_, "visualization/res_plot_traj", par_.res_plot_traj);
  safeGetParam(nh1_, "visualization/color_type_student", par_.color_type_student);
  safeGetParam(nh1_, "visualization/color_type_expert", par_.color_type_expert);
  safeGetParam(nh1_, "visualization/n_agents", par_.n_agents);
  safeGetParam(nh1_, "visualization/z_goal_when_using_rviz", par_.z_goal_when_using_rviz);

  //
  // optimization
  //

  safeGetParam(nh1_, "optimization/c_smooth_yaw_search", par_.c_smooth_yaw_search);
  safeGetParam(nh1_, "optimization/c_visibility_yaw_search", par_.c_visibility_yaw_search);
  safeGetParam(nh1_, "optimization/c_maxydot_yaw_search", par_.c_maxydot_yaw_search);
  safeGetParam(nh1_, "optimization/c_pos_smooth", par_.c_pos_smooth);
  safeGetParam(nh1_, "optimization/c_yaw_smooth", par_.c_yaw_smooth);
  safeGetParam(nh1_, "optimization/c_fov", par_.c_fov);
  safeGetParam(nh1_, "optimization/c_final_pos", par_.c_final_pos);
  safeGetParam(nh1_, "optimization/c_final_yaw", par_.c_final_yaw);
  safeGetParam(nh1_, "optimization/c_total_time", par_.c_total_time);
  safeGetParam(nh1_, "optimization/lambda_obst_avoidance_violation", par_.lambda_obst_avoidance_violation);
  safeGetParam(nh1_, "optimization/lambda_dyn_lim_violation", par_.lambda_dyn_lim_violation);
  safeGetParam(nh1_, "optimization/num_of_intervals", par_.num_of_intervals);

  //
  // other
  //

  safeGetParam(nh1_, "other/factor_alloc", par_.factor_alloc);
  safeGetParam(nh1_, "other/alpha_shrink", par_.alpha_shrink);
  safeGetParam(nh1_, "other/norminv_prob", par_.norminv_prob);
  safeGetParam(nh1_, "other/print_graph_yaw_info", par_.print_graph_yaw_info);
  safeGetParam(nh1_, "other/mode", par_.mode);
