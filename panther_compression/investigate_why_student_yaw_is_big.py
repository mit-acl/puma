...
if __name__ == "__main__":
	...
	path_file=path_panther[:-8]+"/panther_compression/trained_policies/pos_yaw_2000_intermediate_policy_round5_log18.pt"
	# path_file=path_panther[:-8]+"/panther_compression/evals/tmp_dagger/2/intermediate_policy_round14_log28.pt"
	
	# make sure you chage yaw_scaling factor if I am using a different value
	...
	# path_file=path_panther[:-8]+"/panther_compression/evals/tmp_dagger/2/intermediate_policy_round5_log99.pt"
	...
	if use_demo_obs_acts:
		...
		obstacle_position = np.array([[5.0],[0.0],[1.0]])
		goal_position = np.array([[10.0],[0.0],[0.0]])
	...
	acts_n_expert_np = expert_policy.predict(obs_n, deterministic=True)
	...
	acts_n = student_policy.predict(obs_n, deterministic=True)
	...
	w_posBS_student = []
	w_yawBS_student = []
	print("len(acts_n[0]): ", len(acts_n[0]))
	for i in range(len(acts_n[0])):
		...
		print("len(acts_student): ",len(acts_student)) 
		traj_student = venv.am.getTrajFromAction(acts_student, i)
		w_posBS_student_tmp, w_yawBS_student_tmp = venv.am.f_trajAnd_w_State2wBS(traj_student, State(p0, v0, a0, y0, ydot0))

		w_posBS_student.append(w_posBS_student_tmp)
		w_yawBS_student.append(w_yawBS_student_tmp)
	...
	xx_stu = []
	for i in range(len(acts_n[0])):
		xx_stu.append(np.linspace(w_yawBS_student[i].getT0(), w_yawBS_student[i].getTf(), 100))
	ax.set_title('3d pos')
	...
	for i in range(len(acts_n[0])):
		ax.plot(w_posBS_student[i].pos_bs[0](xx_stu[i]), w_posBS_student[i].pos_bs[1](xx_stu[i]), w_posBS_student[i].pos_bs[2](xx_stu[i]), lw=4, alpha=0.7, label=f'student traj #{i}')
	...
	num_vectors = 10
	xx2_exp = np.linspace(w_posBS_expert.getT0(), w_posBS_expert.getTf(), num_vectors)
	xx2_stu = []
	for i in range(len(acts_n[0])):
		xx2_stu.append(np.linspace(w_yawBS_student[i].getT0(), w_yawBS_student[i].getTf(), num_vectors))
	initialized_exp = False # for quiver label
	initialized_stu = False # for quiver label
	# for expert

	for xx2_i in xx2_exp:
	...
	# for student
	for i in range(len(acts_n[0])):
		for xx2_i in xx2_stu[i]:
			xs_student = w_posBS_student[i].pos_bs[0](xx2_i)
			ys_student = w_posBS_student[i].pos_bs[1](xx2_i)
			zs_student = w_posBS_student[i].pos_bs[2](xx2_i)
			yaw_student = w_yawBS_student[i].pos_bs[0](xx2_i)
			xe_student = 1.0 * np.cos(yaw_student)
			ye_student = 1.0 * np.sin(yaw_student)
			ze_student = 0
			if not initialized_stu:
				ax.quiver(xs_student, ys_student, zs_student, xe_student, ye_student, ze_student, color='blue', label='student')
				initialized_stu = True
			else:
	...
	ax[0].plot(xx_exp, w_yawBS_expert.pos_bs[0](xx_exp), 'r-', lw=4, alpha=0.7, label='expert yaw')
	for i in range(len(acts_n[0])):
		ax[0].plot(xx_stu[i], w_yawBS_student[i].pos_bs[0](xx_stu[i]), 'b-', lw=4, alpha=0.7, label=f'student yaw #{i}')
	ax[0].grid(True)
	ax[0].legend(loc='best')

	ax[1].set_title('Derivative of Yaw comparison')
	ax[1].plot(xx_exp, w_yawBS_expert.vel_bs[0](xx_exp), 'r-', lw=4, alpha=0.7, label='expert dyaw')
	for i in range(len(acts_n[0])):
		ax[1].plot(xx_stu[i], w_yawBS_student[i].vel_bs[0](xx_stu[i]), lw=4, alpha=0.7, label=f'student dyaw #{i}')
	ax[1].grid(True)
