...
def readPANTHERparams():
	...
	params_yaml_1=[]
	with open(os.path.dirname(os.path.abspath(__file__)) + '/../../../puma/param/puma.yaml', "r") as stream:
		try:
			params_yaml_1=yaml.safe_load(stream)
		except yaml.YAMLError as exc:
			print(exc)
	params_yaml_2=[]
	with open(os.path.dirname(os.path.abspath(__file__)) + '/../../../puma/matlab/casadi_generated_files/params_casadi.yaml', "r") as stream:
		try:
...
class ObstaclesManager():
	...
	def getSizeAllObstacles(self):
		...
		return self.num_max_of_obst*(3*self.getCPsPerObstacle() + 3)
...
class OtherAgentsManager():
	...
	def __init__(self, policy):
		...
		for i in range(self.num_max_of_obst):
...
class ObservationManager():
	def __init__(self):
		...
		self.num_max_of_obst = params["num_max_of_obst"]
...
class StudentCaller():
	...
	def predict(self, w_init_ppstate, w_ppobstacles, w_gterm, num_obst, num_oa): #pp stands for py_panther
		...
		for i in range(num_obs, self.num_max_of_obst):
