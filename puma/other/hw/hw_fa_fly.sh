...
tmux resize-pane -t $SESSION:$w.1 -x 90

# create 7 panes for each agent (column)
for i in 1 2 2 2 2 2 8 8 8 8 8 8
do 
...
do
	...
	if [ $i -eq 1 ] || [ $i -eq 8 ];
	then
		tmux resize-pane -t $SESSION:$w.$i -y 70
	else
		tmux resize-pane -t $SESSION:$w.$i -y 5
	fi
done
