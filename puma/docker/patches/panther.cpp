                     ...
                     int& k_index_end)
{
  ...
  else
  {
    ...
    // (pop_back keeps the front), the optimizer is fed the NEAREST (most imminent)
    // obstacle. Tested vs highest-collision-probability sorting -- nearest-first is more
    // reliable here because the nearest obstacle is the imminent threat; feeding a farther
    // high-probability obstacle lets the near one clip. (The original code sorted all_probs
    // ascending, keeping the LEAST-dangerous obstacle -- a bug.)
