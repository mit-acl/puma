                                         ...
                                         double tf)
{
  ...
  {
    // DYNAMIC tracking guess: at each yaw control point j, aim from the drone's guessed
    // position at that phase toward the nearest obstacle's position at that phase. This
    // makes the camera smoothly TURN to follow the obstacle across the trajectory (visible
    // tracking), which the optimizer keeps because c_fov=0 and c_yaw_smooth is tiny. The
    // running unwrap keeps it a smooth continuous sweep (no +-pi jumps).
    int Np = matrix_qp_guess.columns();
    int No = obstacles_for_opt_[0].ctrl_pts.size();
    casadi::DM yaw_guess(1, sy_.N + 1);
    double prev = y0;
    for (int j = 0; j <= sy_.N; j++)
    {
      double frac = (sy_.N > 0) ? (double)j / (double)sy_.N : 0.0;
      int pi = (int)(frac * (Np - 1) + 0.5);  if (pi < 0) pi = 0;  if (pi > Np - 1) pi = Np - 1;
      int oi = (int)(frac * (No - 1) + 0.5);  if (oi < 0) oi = 0;  if (oi > No - 1) oi = No - 1;
      Eigen::Vector3d dp(double(matrix_qp_guess(0, pi)), double(matrix_qp_guess(1, pi)),
                         double(matrix_qp_guess(2, pi)));
      Eigen::Vector3d op = obstacles_for_opt_[0].ctrl_pts[oi];
      double yaw = atan2(op.y() - dp.y(), op.x() - dp.x());
      yaw = prev + wrapFromMPitoPi(yaw - prev);  // unwrap from previous -> smooth sweep
      yaw_guess(0, j) = yaw;
      prev = yaw;
    }
    return yaw_guess;
  ...
  casadi::DM constant_yaw_matrix_casadi = y0 * casadi::DM::ones(1, sy_.N + 1);
